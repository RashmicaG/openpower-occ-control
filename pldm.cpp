...
int Interface::setupPldm()
{
    ...
    // Create PLDM stuffs
    pldmReq = pldm_requester_init();
    if (!pldmReq) {
        log<level::ERR>(
            fmt::format(
                "setupPldm: Failed to connect to init pldm_transport, errno={}/{}",
                openErrno, strerror(openErrno))
                .c_str());
        return -1;
    }
    ...
    mctpTransport 
    = (pldm_transport_mctp_demux *)pldm_transport_mctp_demux_init();
    if (!mctpTransport) {
        pldm_requester_destroy(pldmReq);
        log<level::ERR>(
            fmt::format(
    ...
    auto rc = pldm_requester_register_transport(pldmReq,
                                 pldmTransport);
    if (rc != PLDM_REQUESTER_SUCCESS) {
        ...
        log<level::ERR>(
            fmt::format(
                "setupPldm: Failed to connect register transport, errno={}/{}",
                openErrno, strerror(openErrno))
                .c_str());
        return -1;
    }
...
void Interface::pldmClose()
{
    ...
    pldm_transport_mctp_demux_destroy(mctpTransport);
    pldm_requester_unregister_transports(pldmReq);
    pldm_requester_destroy(pldmReq);
    pldmReq = NULL;
    pldmFd = -1;
    eventSource.reset();
